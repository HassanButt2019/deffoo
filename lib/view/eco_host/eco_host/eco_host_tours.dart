...
import 'package:deffo/model/tour_model.dart';
...
class EcoHostTours extends StatefulWidget {
  const EcoHostTours({Key? key}) : super(key: key);

  @override
  _EcoHostToursState createState() => _EcoHostToursState();
}

class _EcoHostToursState extends State<EcoHostTours> {
                  ...
                  onTap: () async{
                    var result = await    Navigator.push<dynamic>(
                      ...
                      MaterialPageRoute<dynamic>(
                        builder: (BuildContext context) => EcoFeelDetails("complete_tour_booking","confirm_cancle_tour_booking",model:bookList[index],imagePath: imagePath+bookList[index].image.toString(),),
                      ),
                    );
                    if(result=="yes"){
                      setState(() {
                    ...
                    child: Column(
                      children: [
                        ...
                        ListTile(
                          ...
                          subtitle: Text(
                              "Tour Name : ${bookList[index].tourName}"),
                        )
                      ],
                    ),
                  ),
  ...
  getBooking() async {
    ...
    Map response = await apiBaseHelper.postAPICall(
        Uri.parse(
            "https://alphawizztest.tk/Deffo/api/Authentication/tour_booking_by_host_id"),
        param);
    setState(() {
      status = false;
    });
    ...
    if (response['status']) {
      for(var v in response['data']){
        setState(() {
          ...
          bookList.add(TourModel.fromJson(v));
        });
      }
    }
  }
