...
import 'package:deffo/model/cycling_model.dart';
import 'package:deffo/model/festival_model.dart';
...
class EcoHostFestival extends StatefulWidget {
  const EcoHostFestival({Key? key}) : super(key: key);

  @override
  _EcoHostFestivalState createState() => _EcoHostFestivalState();
}

class _EcoHostFestivalState extends State<EcoHostFestival> {
                  ...
                  onTap: () async{
                    var result = await   Navigator.push<dynamic>(
                      ...
                      MaterialPageRoute<dynamic>(
                        builder: (BuildContext context) => EcoFeelDetails("complete_festival_booking","confirm_cancle_festival_booking",model:bookList[index],imagePath: imagePath+bookList[index].image.toString(),),
                      ),
                    );;
                    if(result=="yes"){
                      setState(() {
                    ...
                    child: Column(
                      children: [
                        ...
                        ListTile(
                          ...
                          subtitle: Text(
                              "No. Of Day : ${bookList[index].noOfDay}\nActivity Name : ${bookList[index].activityName}"),
                        )
                      ],
                    ),
                  ),
  ...
  getBooking() async {
    ...
    Map response = await apiBaseHelper.postAPICall(
        Uri.parse(
            "https://alphawizztest.tk/Deffo/api/Authentication/festival_booking_by_host_id"),
        param);
    setState(() {
      status = false;
    });
    ...
    if (response['status']) {
      for(var v in response['data']){
        setState(() {
          ...
          bookList.add(FestivalModel.fromJson(v));
        });
      }
    }
  }
