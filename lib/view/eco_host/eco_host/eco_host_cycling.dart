...
class EcoHostCycling extends StatefulWidget {
  const EcoHostCycling({Key? key}) : super(key: key);

  @override
  _EcoHostCyclingState createState() => _EcoHostCyclingState();
}

class _EcoHostCyclingState extends State<EcoHostCycling> {
                  ...
                  onTap: () async{
                    var result = await   Navigator.push<dynamic>(
                      ...
                      MaterialPageRoute<dynamic>(
                        builder: (BuildContext context) => EcoFeelDetails("complete_cycle_booking","confirm_cancle_cycle_booking",model:bookList[index],imagePath: imagePath+bookList[index].image.toString(),),
                      ),
                    );
                    if(result=="yes"){
                      setState(() {
                    ...
                    child: Column(
                      children: [
                        ...
                        ListTile(
                          ...
                          subtitle: Text(
                              "No. Of Bikes : ${bookList[index].noOfBikes}\nBike Name : ${bookList[index].bikeName}"),
                        )
                      ],
                    ),
                  ),
  ...
  getBooking() async {
    ...
    Map response = await apiBaseHelper.postAPICall(
        Uri.parse(
            "https://alphawizztest.tk/Deffo/api/Authentication/cycle_booking_by_host_id"),
        param);
    setState(() {
      status = false;
    });
    ...
    if (response['status']) {
      for(var v in response['data']){
        setState(() {
          ...
          bookList.add(CyclingModel.fromJson(v));
        });
      }
    }
  }
