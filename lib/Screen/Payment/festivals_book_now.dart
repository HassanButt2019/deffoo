...
class FestivalBooking extends StatefulWidget {
  final bookData;
  final type;
  final startDate;
  final endDate;
  final memberNumber;
  const FestivalBooking({Key? key, this.bookData, this.type, this.startDate, this.endDate, this.memberNumber}) : super(key: key);

  @override
  State<FestivalBooking> createState() => _FestivalBookingState();
}

class _FestivalBookingState extends State<FestivalBooking> {
          ...
          children: [
            ...
            InkWell(
                onTap: ()async{
                  ...
                  var price = int.parse(widget.bookData.rentPerPerson!=""&&widget.bookData.rentPerPerson!=null?widget.bookData.rentPerPerson:"10") * _n;
                  var data = widget.bookData;
                  var userId = user;
                  var hostId = "${data.userId}";
                  var festivalId = "${data.id}";
                  var festivalName = "${data.activityName}";
                  ...
                  var payment = "Offline";
                  var noOfGuest = "$_n";
                  BookingModal? modal = await festivalBooking(userId, hostId, festivalId, festivalName, customerName, email, phone, date, amount, payment, noOfGuest);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        title: Wrap(
          children: [
            Padding(
              ...
              child: Text(
                "Amount ₹ ${int.parse(widget.bookData.rentPerPerson!=""&&widget.bookData.rentPerPerson!=null?widget.bookData.rentPerPerson:"10")*_n}",
        ...
        trailing: Padding(
          ...
          child: ElevatedButton(
              ...
              onPressed: () {
                ...
                _bookNow();
                // Navigator.push(context, MaterialPageRoute(builder: (context)=> HotelSummary()));
              },
              child: Text(
                "Make Payment",
      ...
      appBar: AppBar(title: Text("FestivalBooking"), elevation: 0.0),
      body: Center(
          child: Container(
