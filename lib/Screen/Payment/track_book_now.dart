...
class TrackBooking extends StatefulWidget {
  ...
  final endDate;
  final memberNumber;
  const TrackBooking({Key? key, this.bookData, this.type, this.startDate, this.endDate, this.memberNumber}) : super(key: key);
...
class _TrackBookingState extends State<TrackBooking> {
          ...
          children: [
            ...
            InkWell(
                onTap: ()async{
                  ...
                  var trackId = "${data.id}";
                  var trackName = "${data.trackStartPoint} - ${data.trackEndPoint}";
                  ...
                  var payment = "Offline";
                  var noOfGuest = "$_n";
                  BookingModal? modal = await trackBooking(hostId, userId, trackId, trackName,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        title: Wrap(
          children: [
            Padding(
              ...
              child: Text(
                "Amount ₹ ${widget.bookData.ratePerPerson!=""&&widget.bookData.ratePerPerson!=null?int.parse(widget.bookData.ratePerPerson) * _n:"0"}",
      ...
      appBar: AppBar(title: Text("Tracking_Trailing_Booking"), elevation: 0.0),
      body: Center(
          child: Container(
        ...
        ListTile(title: Text("Date From"), trailing: Text("${start!.day}-${start!.month}-${start!.year}"),),
        ListTile(title: Text("Date To"), trailing: Text("${end!.day}-${end!.month}-${end!.year}"),),
        SizedBox(
          width: 10,
          height: 5,
        ),
